...
In this chapter we are going to explainthe different methods used during this thesis.
...
% TODO: add a paragraph about dilation
...
This will allow us to regress a heatmap representation of a point using the Mean Sequared Error loss function.

The heatmap is built using the coordinate of a point $p = (p_x, p_y)$ and computing a guassian arround this point.
...
Another approach to track a point in a video sequence could consist on training a model which outputs an embedding for each pixel that can be meaningfull to obtain instance segmentation.
% TODO: reformulate
...
In our escenario, we use DeepLab~\deeplab explained in detail in Section~\ref{sec:methods_backbone_architecture}.
...
When the image is forwarded throught the model and the embeddings computed, $N$ triplets of pixels are sampled.
...
Finally, a pixel not belonging to the mask is sampled and assigened as negative pixel $x_n$.
