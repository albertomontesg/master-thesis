...
The DeepLab~\deeplab model applies a sequence of convolutional layers to the input image, and instead of a classifier, plugs some deconvolutional\ToDo{There are no deconv layers for deeplab} operations at the very last part of the architecture to obtain a segmentation mask.
...
\figref{resnet_block} illustrates how each bottleneck layer block is built.
...
Also, note that the number of output channels is 2048, so this output can be used directly, or a header\ToDo{Explain what a header is} can be plugged to reduce the dimensionality (in the case of classification, reduce the dimensionality to the number of classes).
...
In our experiments, we are going to use the DeepLab ResNet architecture described in Section~\ref{sec:methods_backbone_architecture} plus an additional module, a PSP~\pspnet module that will reduce the output to a single channel mask to represent the heatmap using a pyramid scene parsing.\ToDo{Reformulate text}
...
Metric learning performs the task of learning a distance function over objects, which in our case, the objects are embeddings.\ToDo{Reformulate}
...
All these steps can be visualized in \figref{instance_segmentation} where the ground-truth mask and a keypoint per each dog instance, the distance map of the embeddings against the keypoint embedding and finally the predicted mask are displayed.
