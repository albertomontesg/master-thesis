...
The next step is to evaluate the instance segmentation given a point per instance. As there exists a one-to-one matching between the ground truth masks and the predicted masks, the evaluation metric used will be the Mean Intersection over Union (mIoU). This is computed as follows, being ...:
...
Some examples of this procedure are shown at \figref{fig:distance_maps} where the image and the ground truth are show at the left column.
On the middle column are shown the distance maps for different instances in the image and in the right column there are the predicted masks after thresholding the distance masp.
...
% TODO: Show graphical results of segmentations from DAVIS dataset
