...
The results are plotted on \figref{experiments:tracking:pointregression}.
...
As can be seen in \figref{experiments:tracking:metriclearning}, the localization precision is really poor, in comparison with the previous experiments.
In contrast, the coverage results in \tabref{experiments:tracking:coveragemetriclearning} increase once training is done and the dimensionality is being reduced. The last configuration almost achieves the same result that a dedicated network for each sequence.
...
As can be seen in \tabref{experiments:videosegmentation:davismiou}, the segmentation using the ground truth to compute a point in every frame, leads to a similar performance compared with the state of the art.
