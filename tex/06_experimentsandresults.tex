...
Given the ResNet model, we tested this architecture with pretrained weights, performing some training with the Triplet Loss and expanding this architecture with a convolutional head to reduce the embedding dimensions.\ToDo{Rephrase}
...
In \figref{pca_embeddings}, some examples of PASCAL~\pascal validation images are shown.
...
Some examples of results are shown in \figref{distance_maps} where the images and the ground truths are in the left column.
...
In order to test different margin values $\alpha$, the result of several experiments is shown in \tabref{margin_miou}.
...
As can be seen in \tabref{davis_miou}, the segmentation using the ground truth to compute a point in every frame, leads to a similar performance compared with the state of the art.
...
If we further analyze the results, the per sequence performance when tracking the keypoints is shown in \figref{per_sequence_miou}.
...
In addition, the prediction of the mask for different sequences are plotted in \figref{pred_masks_davis}.
